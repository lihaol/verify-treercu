...
#ifdef PER_CPU_DATA_ARRAY
	if (unlikely(rcu_sched_qs_mask[smp_processor_id()]))
#else
...
{
#ifdef PER_CPU_DATA_ARRAY
	return atomic_read(&rcu_dynticks[smp_processor_id()].dynticks) & 0x1;
#else
...
{
#ifdef PER_CPU_DATA_ARRAY
	return rcu_dynticks[smp_processor_id()].dynticks_nesting <= 1;
#else
